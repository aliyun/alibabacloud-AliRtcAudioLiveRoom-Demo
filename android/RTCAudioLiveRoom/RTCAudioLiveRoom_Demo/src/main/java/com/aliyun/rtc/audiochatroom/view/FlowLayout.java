...
public class FlowLayout extends ViewGroup implements GestureDetector.OnGestureListener {
    ...
    //子View的对齐方式
    private int isAlignByCenter = AlienState.LEFT;
    ...
    public <T> void setAdapter(List<? extends T> list, int res, BaseItemView<T> mBaseItemView) {
        if (list == null) {
            return;
        }
        removeAllViews();
        ...
        for (int i = 0; i < size; i++) {
            ...
            mBaseItemView.getCover(item, new ViewHolder(inflate), inflate, i);
            addView(inflate);
        }
    }

    public abstract static class BaseItemView<T> {
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (y >= top && y <= bottom && x >= left
                    && x <= right && selectedPosition != i) {
                view.setSelected(true);
                View previousView = getChildAt(selectedPosition);
                if (previousView != null) {
                    previousView.setSelected(false);
                }
                selectedPosition = i;
                if (mOnItemSelectedChangeListener != null) {
                    mOnItemSelectedChangeListener.onItemSelectedChanged(this, selectedPosition);
                }
                requestLayoutInner();
                break;
            }
    ...
    interface OnItemSelectedChangeListener {
        void onItemSelectedChanged(View view, int position);
    }
