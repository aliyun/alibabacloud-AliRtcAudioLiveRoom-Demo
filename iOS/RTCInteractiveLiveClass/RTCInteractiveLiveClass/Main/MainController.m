...
- (void)share{
    NSString *shareurl =[NSString stringWithFormat:@"%@index.html#/studentOnly?channel=%@&role=1",KShareBaseUrl_InteractiveClass,_authInfo.channel];
...
- (void)joinRTC:(AliRtcAuthInfo*)authInfo{
    ...
    __weak typeof(self) weakSelf = self;
    [NetworkManager GET:[kBaseUrl_InteractiveClass stringByAppendingString:@"describeChannelUsers"] parameters:@{@"channelId":_authInfo.channel} completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        ...
        if (num > 0) {
            [self.rtcManager joinChannel:authInfo name:weakSelf.userName success:^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [RTCHUD showHud:@"连麦模式" inView:self.view];
                    
                });
            } error:^(NSInteger errCode) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf.isJoinChannel = NO;
                    [RTCHUD showHud:@"连麦失败" inView:self.view];
                });
            }];
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.isJoinChannel = NO;
                [RTCHUD showHud:@"连麦失败,老师已离开" inView:self.view];
            });
        }
    }];
...
/// 旁路直播模式观看
/// @param authInfo  授权信息
- (void)joinByUrl:(AliRtcAuthInfo*)authInfo {
    self.remoteUserView.hidden = YES;
    self.networkLabel.text =@"";
    [self removeRTCSDK];
    [NetworkManager GET:[kBaseUrl_InteractiveClass stringByAppendingString:@"getPlayUrl"] parameters:@{@"channelId":authInfo.channel,@"userId":authInfo.user_id} completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)onPublishResult:(int)result isPublished:(BOOL)isPublished {
    if (result > 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [RTCHUD showHud:@"推流失败" inView:self.view];
        });
